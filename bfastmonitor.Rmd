...
history_roc <- function(formula, data, level = 0.05) {
  ...
  # the start of the history period is set to the first index (all the data until to the beginning
  #of the monitoring period is stable).
  y_start <- if(sctest(y_rcus)$p.value < level) {
    # Number of all measurements minus the minimum index of the set of measurements of the CUSUM process that exeeds the threshold/boundary.
