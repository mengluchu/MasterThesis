    ...
    verbose = FALSE, plot = FALSE) 
{
    ...
    # checks whether data is a ts class
    if (!is.ts(data)) 
        data <- as.ts(data)
    
    # returns the number of samples per unit time and
    # deltat the time interval between observation
    freq <- frequency(data)

    # L is used as a suffix to qualify any number with
    # the intent of making it an explicit integer. It is useed 
    # to get the code to run faster and consume less memory.
